...
| <img src="./Icons/Architecture-Service/Arch_Compute/64/Arch_AWS-Lambda_64.png" width="40"> | [**AWS Lambda**](https://aws.amazon.com/lambda/) | | Function as a Service (FaaS) to run code on-demand without provisioning or managing underlying servers (Servlerless).<br>Pay per request and compute time. It is an event driven serverless compute service. It's possible to get more resources per function (RAM, CPU, Network).<br>It's not automatically exposed as a REST API (see **API Gateway** in [Application Integration](#application-integration)). |
